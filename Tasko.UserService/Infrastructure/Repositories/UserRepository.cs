...
internal class UserRepository : UserRepositoryBase, IUserRepository
{
    ...
    await (await UserCollection.FindAsync(_ => true)).ToListAsync(cancellationToken);
    ...
    await (await UserCollection.FindAsync(expression)).SingleOrDefaultAsync(cancellationToken);
